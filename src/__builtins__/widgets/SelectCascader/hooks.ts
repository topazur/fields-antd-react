...
import type { ICascaderEvent, ICascaderProps, ICascaderResponse } from './types'

const cascaderEvent: ICascaderEvent[] = ['mount', 'popup', 'search', 'load', 'loupe']

/**
 * @title 接受 request 并处理统一异步逻辑
 */
export function useCascaderRequest(props: ICascaderProps) {
  const { request, pickEvent, omitEvent } = props

  /** 请求状态 (beforeEvent 是触发 Event 之前的临界状态) */
  const [requestStatus, setRequestStatus] = useState<ICascaderEvent | 'beforeSearch' | false>(false)
  ...
  const allowableEvents = useMemo(() => {
    ...
    const events = aPickEvent.length > 0
      ? cascaderEvent.filter(item => aPickEvent.includes(item))
      : aOmitEvent.length > 0
        ? cascaderEvent.filter(item => !aOmitEvent.includes(item))
        : []

    const obj = cascaderEvent.reduce<Record<ICascaderEvent, boolean>>((prev, cur) => {
  ...
  const onRequestHandler = useMemoizedFn(
    (type: ICascaderEvent, params?: Record<string, any> | null): Promise<ICascaderResponse['content']> => {
