...
import type { ISelectTreeResponse } from '../'
...
export function generatePromise(args: ISelectTreeResponse): Promise<ISelectTreeResponse> {
...
const App: React.FC = (props) => {
  return (
    <div {...props}>
      <span style={{ fontSize: '14px', color: '#aaa' }}>仍可以直接使用 treeData，但是优先级低于远程数据模式</span>
      <span style={{ fontSize: '14px', color: '#aaa' }}>仍可以直接使用 filterTreeNode，但是优先级低于远程数据模式</span>
      <span style={{ fontSize: '14px', color: '#aaa' }}>`multiple=true` 或者 `开启远程搜索` 时，默认开启 showSearch 属性</span>
      <span style={{ fontSize: '14px', color: '#aaa' }}>默认开启 treeDataSimpleMode 属性。优点: 扁平数据便于管理，内部会先转化成树形结构再传递给 AntdSelectTree 组件</span>

      <Divider orientation="left">组件库自带搜索功能: treeData + filterTreeNode</Divider>
      <SelectTree
        treeDataSimpleMode={false}
        treeData={genTreeData(2, 4, 'Tree-', true)}
        showSearch={true}
        filterTreeNode={true}
